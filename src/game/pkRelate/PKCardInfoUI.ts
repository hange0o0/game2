        ...
        else
        {
            if(this.dataIn.sp)
            {
                ...
                if(skillNum >= CRM.maxSkill)
                    arr1.push({index:1,icon:'icon_atk_png',iconScale:1,title:'使用次数',rate:1,value:'无限'})
                else
                    arr1.push({index:1,icon:'icon_atk_png',iconScale:1,title:'当前拥有',value:skillNum + ' /' + CRM.maxSkill,rate:skillNum/CRM.maxSkill})
                if(this.dataIn.sp.num)
                {
                    arr1.push({index:2,icon:'icon_love_png',iconScale:0.6,title:'可兑换数量',value:this.dataIn.sp.num})
                }
            }
